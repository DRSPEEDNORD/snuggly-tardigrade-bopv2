...
import PWAInstaller, { PWAInstallerRef } from './PWAInstaller';

const ActionCards = () => {
  const pwaInstallerRef = useRef<PWAInstallerRef>(null);
  ...
  const handleInstallClick = (e: React.MouseEvent) => {
    e.preventDefault();
    pwaInstallerRef.current?.show();
  };

  return (
    <>
      <PWAInstaller ref={pwaInstallerRef} />
      ...
      >
        {/* Carte Installer l'App - Maintenant déclenche le dialogue PWA */}
        <motion.button
          variants={item}
          onClick={handleInstallClick}
          className="relative group overflow-hidden rounded-[2rem] bg-black/40 border border-white/5 hover:border-emerald-500/30 transition-all duration-500 p-6 text-left w-full"
        >
